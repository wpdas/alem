...
const replacePathAlias = (path) => {
  pathsKeys.forEach((pathAlias) => {
    ...
    // Windows adjusment
    const fixedPathAlias = isWindows ? pathAlias.replaceAll("/", "\\") : pathAlias;

    if (path.includes(fixedPathAlias)) {
      // path alias corrigido para funcionar em Windows, OSX e Linux
      const osRelativePathAlias = isWindows ? paths[pathAlias].relative.replaceAll("/", "\\") : paths[pathAlias].relative;

      path = path.replace(fixedPathAlias, osRelativePathAlias);
    }
  });
...
const hasPathAlias = (path) => {
  ...
  pathsKeys.forEach((pathAlias) => {
    // Windows adjusment
    pathAlias = isWindows ? pathAlias.replaceAll("/", "\\") : pathAlias;
    //ex: path includes @app
