...
const parseAlemFeatures = require("./parseAlemFeatures");
      ...
      if (importItemFilePath && !pastedFiles.includes(importItemFilePath)) {
        ...
        if (isImportItemCommingFromJsxFile) {
          ...
          if (importItemFileSource) {
            ...
            const importItemWidgetName = fileSchemas.find(
              (importFileSchema) =>
                importFileSchema.filePath === importItemFileSource,
            ).widgetName;
            ...
            // INFO:
            // Se for o src/index, usa:
            // props={{ alem: globalProps.alem }}
            // Se nao for arquivo src/index, usa:
            // props={{ text: "any test prop here", alem: props.alem }}
            // "alem: props.alem"
            // TODO: injetar as propriedades do proprio componente
            if (fileSchema.filePath.includes("src/index")) {
              fileBundle = replaceWordAfterTheLessThanSign(
                fileBundle,
                importItemWidgetName,
                `<Widget code={props.alem.componentsCode.${importItemWidgetName}}`,
              );
            } else {
              fileBundle = replaceWordAfterTheLessThanSign(
                fileBundle,
...
const transformSchemaToWidget = (fileSchemas) => {
  ...
  // ex: "useStore" se tornará "props.alem.useStore"
  fileSchemas.forEach((fileSchema, fileSchemaIndex) => {
    fileSchema.content = parseAlemFeatures(fileSchema.content);
