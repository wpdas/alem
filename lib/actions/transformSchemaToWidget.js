...
const processSchema = (fileSchema) => {
  ...
  console.log(fileSchema.filePath, isIndex);
  ...
  // Se o componente for statefull, mesmo tendo o nome .module., ele é definido como não module
...
const swapComponentsForStatelessFiles = (fileSchemas, fileSchema) => {
  ...
  // if (!isModule) {
  // if (_hasHtmlElements && !hasWidgetProperties && isModule) {
    ...
    if (importItemFilePath && !pastedFiles.includes(importItemFilePath)) {
      ...
      if (isImportItemCommingFromJsxFile) {
        ...
        if (importItemFileSource) {
          ...
          // INFO: Recursividade
          // if (!importItemWidget.swapComponentsForStatelessFiles_Done) {
          //   // Faz o processo primeiro no arquivo dependente

          //   // updated "importItemFileContent"
          //   // importItemWidget = swapComponentsForStatelessFiles(
          //   //   fileSchemas,
          //   //   importItemWidget,
          //   // );

          //   console.log(
          //     "NAO FOI PROCESSADO AINDA:",
          //     fileSchema.filePath,
          //     importItemWidget.filePath,
          //   );
          // }

          // if (!importItemWidget.injectFilesDependencies_Done) {
          //   // Faz o processo primeiro no arquivo dependente

          //   // updated "importItemFileContent"
          //   importItemWidget = injectFilesDependencies(
          //     fileSchemas,
          //     importItemWidget,
          //   );
          ...
          //   getComponentName(importItemWidget.content) ||
          //   importItemWidget.widgetName;

          // TODO: Tem um erro no getComponentName, testar as linhas comentadas acima
          const importItemWidgetComponentName =
            importItemWidget.widgetName ||
          ...
          //   "| Teste name com getComponentName():",
          //   getComponentName(importItemWidget.content),
          //   "| Item file source:",
          //   importItemFileSource,
          //   "| Item is module?",
          //   importItemWidget.isModule,
          // );
          // if (importItemFileSource === "src/pages/About/About.tsx") {
          //   console.log(importItemWidget.finalFileBundle);
          // }
    ...
    if (importItemFilePath && !pastedFiles.includes(importItemFilePath)) {
      ...
      if (importItemFileContent.isModule) {
        ...
        if (importItemFileSource) {
          ...
          //   a.forEach((aItem) => {
          //     if (!has) {
          //       has = b.includes(aItem);

          //       if (has) {
          //         console.log("CONFLICT FOUND:", aItem);

          //         const conflictedContent = fileSchemas.find(
          //           (item) => item.filePath === aItem,
          //         );

          //         console.log(
          //           "Conteudo no bundle?",
          //           // conflictedContent.finalFileBundle,
          //           fileSchema.finalFileBundle.includes(
          //             conflictedContent.finalFileBundle,
          //           ),
          //         );

          //         // Se achar o conteúdo duplicado no bundle do fileSchema, remove ele
          //         if (fileBundle.includes(conflictedContent.finalFileBundle)) {
          //           fileBundle = fileBundle.replace(
          //             conflictedContent.finalFileBundle,
          //             "",
          //           );
          //         }
          ...
          //   ) ||
          //   foo(importItemFileContent.injectedFiles, fileSchema.injectedFiles)
          // ) {
          //   console.log(
          ...
          // INFO: Verifica se tem conteúdo inserido com duplicidade e trata isso
          // fileBundle = removeDuplicateInjection(
          //   fileBundle,
          //   fileSchemas,
          //   fileSchema,
          ...
          //   !fileSchema.injectedFiles.includes(importItemFileContent.filePath)
          // ) {
          //   fileSchema.injectedFiles.push(importItemFileContent.filePath);
          // }
          ...
          // console.log("INSERINDO Injeçoes para:", fileSchema.filePath);
          // console.log("Inserindo:", importItemFileContent.injectedFiles);
          // console.log("Injeçoes Final:", fileSchema.injectedFiles);
          // console.log("\n");
          ...
          //   (item) =>
          //     fileSchema.injectedFiles.filter(
...
const prepareAlemDependencies = (fileSchema) => {
  // const importStatements = getImportStatements(fileSchema.content);
  console.log("\n\n");
  // console.log("Import Statements ", fileSchema.filePath, importStatements);
  ...
  importItems.forEach((item) => {
    ...
    console.log("BUUU:", item, importStatementFileSource);
...
${fileContent}
  ...
  if (contentChanged) {
    ...
    // console.log(fileSchema.content);
  }
      ...
      if (importItemFilePath && !pastedFiles.includes(importItemFilePath)) {
        ...
        if (isImportItemCommingFromJsxFile && !isModule) {
          ...
          if (importItemFileSource) {
            ...
            // if (!alemImportItemWidgetName) {
            // Le o nome do Widget dependente (usando o fileSchema dele)
            let importItemWidgetName = fileSchemas.find(
              (importFileSchema) =>
                importFileSchema.filePath === importItemFileSource,
            ).widgetName;
            // } else {
