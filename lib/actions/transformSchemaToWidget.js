...
const processSchema = (fileSchema) => {
  ...
  // É um módulo ou arquivo stateless?
  const isModule =
  ...
  if (fileSchema.componentParamsItems) {
    jsContent = `
...
const injectFilesDependencies = (fileSchemas, fileSchema) => {
  ...
  Object.keys(fileSchema.componentImportItems).forEach((importItem) => {
    ...
    if (importItemFilePath && !pastedFiles.includes(importItemFilePath)) {
      ...
      // É um modulo ou arquivo stateless?
      const isModule =
        importItemFilePath.includes(".module.") ||
        !hasWidgetPropsCheck(fileSchema.content);
      ...
      if (!isImportItemCommingFromJsxFile || isModule) {
        ...
        if (importItemFileSource) {
          ...
          if (
            ...
            // Se a dependencia já tiver sido inserida pelo dependente ou sub-dependente, pula
            return;
          }
