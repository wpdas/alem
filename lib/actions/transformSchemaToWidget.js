      ...
      if (importItemFilePath && !pastedFiles.includes(importItemFilePath)) {
        ...
        if (isImportItemCommingFromJsxFile) {
          ...
          if (importItemFileSource) {
            ...
            // Le os dados do arquivo já processado (usando o fileSchema dele)
            // Widget.code (code deste widget que o arquivo depende)
            const importItemWidgetCode = fileSchemas.find(
              (importFileSchema) =>
                importFileSchema.filePath === importItemFileSource,
            ).finalFileBundle;
            ...
            //   fileSchema.filePath,
            //   importItemFileSource,
            //   importItemWidgetCode,
            // );

            console.log(
              `======= Import Widget File Source =========> FROM: ${fileSchema.widgetName} TO: ${importItem} => ${importItemFileSource} => ${importItemWidgetCode}`,
            );
            // console.log(importItemFileContent);
            console.log("\n\n");
            // TODO: Fazer com que todos os arquivos encapsulem o conteúdo de suas dependencias em si mesmo usando o "finalFileBundle"
            fileBundle = `
            $const ${importItem} = \`${scapeBacktick(importItemWidgetCode)}\`
            

            ${fileBundle}
            `;

            // Troca os nomes dos componentes dessa dependencia por <Widget code={NomeComponente} props={...} />
            // ACHAR E TROCAR:
