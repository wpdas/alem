...
const processSchema = (fileSchema) => {
  ...
  const isAlemToolsFolder = fileSchema.filePath.includes("lib/tools/");
  ...
  if (!isJsxFile || (!isInWidgetFolder && !isAlemToolsFolder)) {
  ...
  // se as dependencias (imports) nao estiverem prontas, nao renderiza o componente
  // NOTE: isso é porque alguns componentes (imports) nao estam prontos no momento do render
  ...
  let elementsCheck = "";
  if (!isAlemToolsFolder && isInWidgetFolder) {
    elementsCheck = "if (";
    componentImports.forEach((element, elementIndex) => {
      elementsCheck += `!${element}`;
      if (componentImports.length - 1 === elementIndex) {
        elementsCheck += `) return <></>`;
      } else {
        elementsCheck += " || ";
      }
    });
