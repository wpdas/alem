...
const { MORE_THAN_ONE_SPACE, ALL_BLANK_LINES } = require("../regexp");
    ...
    for (let i = itemsToRemove[filePath]; i > 0; i--) {
      ...
      if (
        ...
        currentFileBundleContent = currentFileBundleContent
          .trim()
          .replace(conflictedContent.finalFileBundle.trim(), "");
        ...
        if (
          ...
          log.error(
            `Item has been completely removed! Inserting item again... (This appears to be an issue with the injectFilesDependencies function which is putting more injections into the list than into the final bundle.)`,
          );
